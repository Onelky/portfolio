...
const Contact = (): JSX.Element => {
  ...
  return (
    <Grid container component={'section'} id="contact" direction="column" alignItems={'center'} mt={10} mb={10}>
      <SectionHeader title={'Contact'} linePosition={LinePositions.rightLeft} justifyContent={'start'} />
      ...
      <form onSubmit={handleSubmit(onSubmit)} style={{ width: '100%', display: 'flex', justifyContent: 'center' }}>
        <Stack width={{ xs: '100%', sm: 550 }} spacing={4} alignItems={'center'}>
          <Controller
            name="name"
            control={control}
            rules={{ required: 'Required', maxLength: { value: 80, message: 'Name cannot exceed 80 characters' } }}
            render={({ field, fieldState: { error } }) => <StyledTextField variant="outlined" label="Name" error={!!error} helperText={error && error.message} {...field} />}
          />
          <Controller
            name="email"
            control={control}
            ...
            render={({ field, fieldState: { error } }) => <StyledTextField variant="outlined" label="Email" error={!!error} helperText={error && error.message} {...field} />}
          />
          <Controller
            name="subject"
            control={control}
            rules={{ required: 'Required', maxLength: { value: 80, message: 'Subject cannot exceed 80 characters' } }}
            render={({ field, fieldState: { error } }) => <StyledTextField variant="outlined" label="Subject" error={!!error} helperText={error && error.message} {...field} />}
          />
          <Controller
            name="message"
            control={control}
            ...
            render={({ field, fieldState: { error } }) => (
              <StyledTextField variant="outlined" label="Message..." multiline rows={4} error={!!error} helperText={error && error.message} {...field} />
            )}
          />
